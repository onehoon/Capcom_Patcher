...
#include "DbgUiRemoteBreakinWatcher.h"
...
// See THIRD_PARTY_NOTICES.md for the applicable notices.
                       ...
                       bool logDetails)
{
    ...
    if (!RevalidateHook(expectedHook, currentHook) || currentHook.target != expectedHook.target)
    {
        if (logDetails)
        {
            Log("[CapcomPatcher][DbgUi] hook changed before neutralization; retrying later");
        }
        return false;
    }
        ...
        !SameRegion(expectedRegion, currentRegion))
    {
        if (logDetails)
        {
            Log("[CapcomPatcher][DbgUi] target region changed before neutralization; retrying later");
        }
        return false;
    }
    ...
    if (!VirtualProtect(currentRegion.BaseAddress, currentRegion.RegionSize, PAGE_EXECUTE_READWRITE, &oldProtection))
    {
        if (logDetails)
        {
            Log("[CapcomPatcher][DbgUi] failed to change memory protection");
        }
        return false;
    }
    ...
    if (!wrote || !restoredProtection || !flushed)
    {
        if (logDetails)
        {
            Log("[CapcomPatcher][DbgUi] failed to neutralize executable private payload");
        }
        return false;
    }
    ...
    {
        Log("[CapcomPatcher][DbgUi] neutralized executable private payload");
...
bool RestoreEntry(const HookInfo& hook)
{
    ...
    {
        Log("[CapcomPatcher][DbgUi] DbgUiRemoteBreakin changed before restore; leaving it untouched");
        return false;
    }
    ...
    {
        Log("[CapcomPatcher][DbgUi] failed to validate DbgUiRemoteBreakin before restore");
        return false;
    }
    ...
    {
        Log("[CapcomPatcher][DbgUi] failed to change DbgUiRemoteBreakin protection");
        return false;
    }
    ...
    if (!wrote || !restoredProtection || !flushed)
    {
        Log("[CapcomPatcher][DbgUi] failed to restore DbgUiRemoteBreakin");
        return false;
    }

    Log("[CapcomPatcher][DbgUi] restored DbgUiRemoteBreakin");
    return true;
}
...
void CheckDbgUiRemoteBreakin()
{
    ...
    if (newlyObserved)
    {
        ...
        Log("[CapcomPatcher][DbgUi] DbgUiRemoteBreakin modification detected");
    }
    ...
    if (!ResolveHook(current, hook))
    {
        if (newlyObserved)
        {
            Log("[CapcomPatcher][DbgUi] unsupported DbgUiRemoteBreakin hook form");
        }
        return;
    }
    ...
    if (newlyObserved)
    {
        Log("[CapcomPatcher][DbgUi] hook type=%s target=%p",
            validatedHook.type,
    ...
    if (!QueryPrivateExecutableTarget(validatedHook.target, targetInformation))
    {
        if (newlyObserved)
        {
            Log("[CapcomPatcher][DbgUi] target is not ready or is not an executable MEM_PRIVATE region");
        }
        return;
    }
    ...
    if (newlyObserved)
    {
        Log("[CapcomPatcher][DbgUi] target region base=%p size=0x%zx type=MEM_PRIVATE protect=0x%08lx",
...
bool Initialize()
{
    ...
    Log("[CapcomPatcher][DbgUi] initializing watcher");

    HMODULE ntdll = GetModuleHandleW(L"ntdll.dll");
    if (ntdll == nullptr)
    {
        Log("[CapcomPatcher][DbgUi] failed to locate ntdll.dll");
        return false;
    }
    ...
    {
        Log("[CapcomPatcher][DbgUi] failed to locate or capture DbgUiRemoteBreakin");
    ...
    {
        Log("[CapcomPatcher][DbgUi] DbgUiRemoteBreakin was already redirected before baseline capture; refusing contaminated baseline");
        g_dbgUiRemoteBreakin = nullptr;
        return false;
    }

    Log("[CapcomPatcher][DbgUi] captured DbgUiRemoteBreakin baseline");
    ...
    if (!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_PIN,
                            ...
                            &pinnedModule))
    {
        Log("[CapcomPatcher][DbgUi] failed to pin watcher module");
        g_dbgUiRemoteBreakin = nullptr;
        return false;
    }
    ...
    {
        Log("[CapcomPatcher][DbgUi] failed to start watcher thread");
        g_dbgUiRemoteBreakin = nullptr;
        return false;
    }
